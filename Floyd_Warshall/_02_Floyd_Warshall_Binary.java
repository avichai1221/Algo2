...
import Bottles_Problem._01_Bottles_Problem;
...
public class _02_Floyd_Warshall_Binary {
	...
	public static void main(String[] args) {
		...
		boolean[][] T4= {
				...
				{true,false,false,true,false,true,false},
				{false,true,true,false,true,false,true}};
		System.out.println(HowManyComponents(T4));

		
		//	System.out.println(isDirectedGraph(T2));;
		//	//with path
	...
	public static int HowManyComponents(boolean matAfterFW[][]) {
		...
		// הדפסת הקודקודים השייכים לאותו רכיב קשירות בשורות
		String cs[] = new String[count];
		for (int i=0; i<count; i++) {
		cs[i] = "";
		}
		for (int i=0; i<size; i++) cs[comps[i]-1] = cs[comps[i]-1]+ i +"\t";
		for (int i=0; i<count; i++) System.out.println(cs[i].toString());
		return count;
	}


	// בדיקה אם הגרף לא מכוון ע"י בדיקה אם המשולש העליון סימטרי למשולש התחתון
	public static boolean isDirectedGraph(boolean mat[][]) {
		int n = mat.length;
		int m = mat[0].length;
		for (int i=0; i<n; i++) {
			for (int j=0; j<m/2; j++) {
				if (mat[i][j]!=mat[n-1-i][m-1-j]) return false;
			}
		}
		return true;
	}
	...
	//  מקבל מטריצת מסלולים (כלומר מפעילים קודם פלויד וורשל!) ---בדיקה אם קיים מסלול בין 2 קודקודים מסויימים
	...
	//  מקבל מטריצת מסלולים (כלומר מפעילים קודם פלויד וורשל!) ---בדיקה אם הגרף קשיר--
	//מספיק לעבור על עמודה אחת ואם כולה אמת זה אומר שכולם מחוברים אליו ולכן קשיר
...
// הדפסת הקודקודים השייכים לאותו רכיב קשירות במערך
